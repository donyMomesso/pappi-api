...
- Taxa de entrega confirmada: R$ ${Number(currentFee).toFixed(2)}
...
- SABORES GENÉRICOS: Se o cliente pedir "frango" e existir mais de um frango no cardápio, liste as opções e pergunte qual prefere.
- PIZZA MEIO A MEIO: Pode. Use o preço do sabor mais caro.
- 1 pergunta por vez.

ROTEIRO:
1) Confirme tamanho + sabores
2) Ofereça borda + bebida
...
`.trim();
    ...
    if (finalOrderPayload) {
      ...
      try {
        ...
        await sendText(from, "Tive um erro ao imprimir o seu pedido na cozinha 😕 Vou chamar um humano pra confirmar com você agora!");
        await setHandoffOn(from);
        return;
      }
    }
